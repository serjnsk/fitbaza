  ...
  items:b.items
    ...
    .map(i => i.exId
      ? { id:i.id, ex:byId(i.exId), scheme:i.scheme, unit:i.unit, val:i.val,
          kg:workKg(i, PM), sets:buildSets(i, PM), done:false, comment:null }
...
/* ─── недельная полоса ───
   Есть не во всех стилистиках: разметку добавляет только та тема,
   которой этот приём принадлежит. Нет контейнера — нет и отрисовки.

   Логика плашек взята из приложения-референса и на первый взгляд
   обратная: серым залит день, в котором НИЧЕГО нет, а тренировочный
   стоит прямо на карточке. Так строка читается с одного взгляда —
   серое значит пусто. Сегодняшний добавляет точку акцентом.

   Дни пока не переключаются: экран собран вокруг одной тренировки,
   и смена дня — это пересборка журнала, а не подсветка кнопки.
   Полоса показывает, где ты в неделе; листание придёт с экраном
   программы целиком. */
const ICO_MOON = `<svg viewBox="0 0 20 20" fill="currentColor"><path d="M16.2 12.4A6.9 6.9 0 0 1 7.6 3.8a6.9 6.9 0 1 0 8.6 8.6z"/></svg>`;
const ICO_PREV = `<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4 6 10l6 6"/></svg>`;
const ICO_NEXT = `<svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M8 4l6 6-6 6"/></svg>`;
function renderWeekNav(){
  const el = $('#wknav'); if(!el) return;
  el.innerHTML = `<div class="wh">${esc(program(PROG).title)}</div>
    <div class="wr">
      <button class="a" aria-label="Прошлая неделя">${ICO_PREV}</button>
      <div class="dd">${W.days.map((x,i)=>`
        <button class="d${x.rest?' rest':''}${i===DAY?' on':''}"
                aria-label="${esc(x.w)}, ${x.rest?'отдых':esc(x.title)}"${i===DAY?' aria-current="date"':''}>
          <s>${esc(x.w)}</s>
          <b>${x.rest?ICO_MOON:D(x.date).getDate()}</b>
        </button>`).join('')}</div>
      <button class="a" aria-label="Следующая неделя">${ICO_NEXT}</button>
    </div>`;
}
...
renderHead(); renderWeekNav(); render();
